...
import swaggerUi from "swagger-ui-express"; // Creates the Swagger UI page (http://localhost:8000/api-docs)
import fs from "fs";
import yaml from "yaml";
import { middleware } from "express-openapi-validator"; // The Express OpenAPI Validator

// Load the OpenAPI spec from the YAML file
const swaggerDocument = yaml.parse(
  fs.readFileSync("src/basicServer.yaml", "utf-8")
);
// the swaggerDocument is the OpenAPI spec.
...
// before every rest endpoint is hit (ex: /incidents), the middleware runs (aka validates the input and output)
...
// URI = "/incidents". Bind the /incidents endpoint to the GET method.
